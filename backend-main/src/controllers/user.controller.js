...
/*
    req body ->data
    validate wherether empty string is pass or not
    validate the username and email
    validate the password
    if the password is right and then provide the refresh token and access token
    provide the refresh token where the user is login in again or not
    send cookies
*/
...
const logoutUser = asyncHandler(async(req,res)=>{
    await User.findByIdAndUpdate(req.user._ud,{
        $set: {
...
const refreshAccessToken = asyncHandler(async (req,res)=>{
   const incomingRefreshToken =  req.cookies.refreshToken || req.body.refreshToken
    ...
    try {
        ...
        return res
        .status.cookie("accessToken",accessToken,options).cookie("refreshToken",newRefreshToken,options).json(
            new ApiResponse(
                200,
...
const changeCurrentPassword = asyncHandler(async(req,res)=>{
   ...
   const user = await User.findById(req.user?._id) //changes to be about to be made // check later
...
const updateUserAvatar = asyncHandler(async(req,res)=>{
  const avatarLocalPath = req.file?.path
    ...
    return res.status(200).json(new ApiResponse(200,"Avatar image updated succesffuly"))
})
...
const updateUserCoverImage = asyncHandler(async(req,res)=>{
    ...
    return res.status(200).json(new ApiResponse(200,"CoveImage updated succesffuly"))
